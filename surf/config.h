...
#define PROMPT_DDG "Search DuckDuckGo:"

/* SETPROP(readprop, setprop, prompt)*/
#define SETPROP(r, s, p, e) { \
        .v = (const char *[]){ "/bin/sh", "-c", \
             "prop=\"$(printf '%b' \"$(xprop -id $1 $2 " \
             "| sed \"s/^$2(STRING) = //;s/^\\\"\\(.*\\)\\\"$/\\1/\")\" " \
             "|  dmenu -fn \"sans:size=10\" -nb \"#3A3A3A\" " \
	     "-nf \"#FFFFFF\" -sb \"#BCBCBC\" -sf \"#000000\" -p \"$4\" -w $1)\" \
	     && xprop -id $1 -f $3 8s -set $3 \"$5$prop\"", \
             "surf-setprop", winid, r, s, p, e, NULL \
        } \
}
...
#define BMARK(p, q) { \
        .v = (const char *[]){ "/bin/sh", "-c", \
             ...
             "| dmenu -fn \"sans:size=10\" -nb \"#3A3A3A\" " \
	     "-nf \"#FFFFFF\" -sb \"#BCBCBC\" -sf \"#000000\" -w $2`\" &&" \
...
#define BMARKSAVE(p, q) { \
        .v = (const char *[]){ "/bin/sh", "-c", \
             "prop=\"`xprop -id $2 $0 " \
             "| sed \"s/^$0(STRING) = \\(\\\\\"\\?\\)\\(.*\\)\\1$/\\2/\" " \
             "| xargs -0 printf %b`\" && echo \"$prop\" >> ~/etc/surf/bookmarks", \
             p, q, winid, NULL \
        } \
}
...
static Key keys[] = {
	...
	{ MODKEY,                GDK_KEY_s,      spawn,      SETPROP("_SURF_URI", "_SURF_GO", PROMPT_DDG, "https://duckduckgo.com/?q=") },
